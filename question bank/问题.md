...
>修饰词 static、private、protect、sealed、final、transient、readonly 、internal、volatile
...
>值类型和引用类型
	...
	值类型是在栈中分配内存，引用类型是在堆中分配内存（字段所需内存、同步块索引、类型对象指针），栈中存储的是变量名和引用类型的引用地址。
...
>string字符串连接，+、concat、string.Format()、StringBuilder
    ...
    1）+：  
    由于字符串的不变性，+操作会重新创建一个新的字符串，多次+操作会频繁创建字符串对象，这时候建议使用StringBuilder;
...
>cap理论
	...
	一致性（consistency）、可用性（Availability）、分区容错（partition-tolerance）不可兼得0
