...
```
答:
	重排: 当DOM元素影响了元素的几何属性（例如宽和高），浏览器需要重新计算元素的几何属性，同样其它元素的几何属性和位置也会因此受到影响。浏览器会使渲染树中受到影响的部分失效，并重新构造渲染树。这个过程称为“重排”。
...
2. 匿名函数中的this总是指向window
3. 定时器中,this指向window
4. 构造函数调用模式, this指向新创建的实例对象
5. 方法调用模式, this指向调用方法的对象
6. 在事件处理函数中,this指向触发事件的当前元素
7. 上下文调用模式, call和apply方法, this指向方法内的第一个参数
                  bind方法, this绑定为bind方法中创建的函数
8. 箭头函数中没有this指向问题,它的this和外层作用域的this保持一致
