...
## 如果让你选择，你会选用什么rpc框架
    1、RPC--指远程过程调用
    .net中的rpc框架包括有remoting（比较老）、webservices（比较老)、wcf(整合了原有的windows通讯的 .net Remoting，WebService，Socket的机     制)、Thrift、Shuttler.Net-RPC/Memcached/IM框架开源发布、gRPC、Halibut、SCS
    twitter的 Finagle，也是netty的深度用户。 https://github.com/twitter/finagle
    阿里的 dubbo   https://github.com/alibaba/dubbo
    大百度到处是C++的天下，于是也有C++的rpc  http://www.oschina.net/p/sofa-pbrpc  也有使用Java的部门跟进的     https://github.com/neoremind/navi
