...
用户通过刷卡、扫码、人脸等方式，获取用户id，查询用户信息，在体重稳定、并且有用户信息的情况下，保存用户的体重信息到系统

* 刷卡、扫码枪、人脸识别

键盘钩子无光标获取扫码枪数据、人脸识别的话是通过人脸识别系统的接口获取最新的人脸信息(设备id)、刷卡是和刷卡设备厂商对接，一般是调用对方的dll
...
使用wpf绘制页面，可以mvvm进行模型绑定，减少直接的控件操作
...
可以考虑下依赖注入得使用，如果是要在wpf中使用依赖注入，该如何操作

wpf用到的点：UserControl(UserControl.Template)、ControlTemplate、ContentPresenter、Binding(动态构建绑定)、RelativeSource(关联数据，用在数据绑定)、StaticResource(一般用在样式和图片等静态资源)

INotifyPropertyChanged(用在绑定模型)、DependencyProperty 用在自定义控件的属性上

`xmlns`:命名空间，可以定义到控件的命名空间，方便调用

`Window.Resources` 资源配置，配置静态资源和样式
...
`FlowDocumentScrollViewer->FlowDocument`:文档流布局
`Table`：表格
