...
`IHostedService`接口如上，主要有两个接口方法，`StartAsync`和`StopAsync`，其实`IHostedService`类似于一个服务管道，注入多个`IHostedService`实现类的时候，会依次执行`StartAsync`方法，程序停止时依次执行`StopAsync`方法，所以这两个方法不能阻塞。如果要以此写个后台服务，`StartAsync`方法内必须把后台服务用`Task`或者`Thread`包装起来，用单独的线程来执行。

`BackgroundService`类就是直接封装好，可以直接拿来用
